...
/// 20.07.2005 more datatypes and XML Documents fixed
  ...
  <xsl:template match="/">
    ...
    // by Matthias Hertel - HyperObjects update done on 2006
  ...
  <xsl:template name="soapMessage">
    ...
    <xsl:for-each select="/wsdl:definitions/wsdl:types/s:schema/s:element[@name=$inputElementName]//s:element">
      <xsl:choose>
        ...
        <!-- xsl:when test="./s:complexType/s:sequence/s:element[@ref='s:schema'] and ./s:complexType/s:sequence/s:any" -->
        <xsl:when test="@type='s:boolean'">
