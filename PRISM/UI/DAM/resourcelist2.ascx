<%@ Control language="c#" Inherits="HyperCatalog.UI.DAM.Controls.ResourceList2" CodeFile="ResourceList2.ascx.cs" CodeFileBaseClass="HyperCatalog.UI.DAM.UserControl" %>
...
<table style="WIDTH: 100%; HEIGHT: 100%" cellspacing="0" cellpadding="0" width="100%" border="0">
	...
	<tr>
		<td id="minimalCell" runat="server" valign="top" height="100%" width="*">
			<ucs:CustomPager id="resourcesPagerTop" runat="server"></ucs:CustomPager>
			<asp:DataGrid id="resourceGrid" runat="server" OnItemDataBound="resourceGrid_ItemCreated" Width="100%" cellpadding="0" cellspacing="0"
				GridLines="Both" ShowFooter="False" ShowHeader="False" AutoGenerateColumns="False" EnableViewState="true"
				AllowPaging="True" PageSize="59" DataKeyField="ResourceId" BorderColor="LightGray">
				...
				<Columns>
					...
					<asp:TemplateColumn>
						<ItemStyle VerticalAlign="top" HorizontalAlign="left"></ItemStyle>
						<ItemTemplate>
							<asp:LinkButton id="lnkEdit" onclick="ChooseResource" runat="server">
								<%# highlightFilter(DataBinder.Eval(Container.DataItem, "ResourceName").ToString(),resourceFilter)%>
							</asp:LinkButton>
							<br/>
							<span style="font-size:80%;font-style:italic;margin-left:5px;">
								<%#highlightFilter(WrapString(DataBinder.Eval(Container.DataItem, "ResourceDescription").ToString(), 30), resourceFilter)%>
							</span>
						</ItemTemplate>
					</asp:TemplateColumn>
				</Columns>
			</asp:DataGrid>
			<ucs:CustomPager id="resourcesPagerBottom" runat="server"></ucs:CustomPager>
			<igtbl:UltraWebGrid id="rGd" Visible="false" runat="server" Width="100%" Height="100%" OnInitializeRow="rGd_InitializeRow">
				<DisplayLayout ColHeadersVisibleDefault="No" AutoGenerateColumns="False" AllowSortingDefault="No"
					...
					RowSelectorsDefault="No" Name="rGd" CellClickActionDefault="RowSelect" NoDataMessage="No resource" LoadOnDemand="Xml">
				...
				<Bands>
					<igtbl:UltraGridBand BorderCollapse="Collapse" DataKeyField="ResourceId">
						<Columns>
							<igtbl:UltraGridColumn Key="Id" BaseColumnName="ResourceId" ServerOnly="true"></igtbl:UltraGridColumn>
							<igtbl:UltraGridColumn Key="Thumbnail" BaseColumnName="URI" Width="40px" ServerOnly="true"/>
							<igtbl:TemplatedColumn Key="Name" BaseColumnName="ResourceName" HeaderText="Name" Width="100%">
								<CellTemplate>
									<asp:LinkButton id="lnkEdit" onclick="SelectResource_Click" runat="server">
										<%# highlightFilter(Container.Value.ToString(),resourceFilter.ToString())%>
									</asp:LinkButton>
								</CellTemplate>
			...
			<td valign="top" width="80%">
				...
				<FIELDSET>
					...
					<igtbar:ultrawebtoolbar id="pTb" runat="server" CssClass="hc_toolbar" width="100%" ItemWidthDefault="70px">
						...
						<SelectedStyle CssClass="hc_toolbarselected"></SelectedStyle>
						<ITEMS>
							<igtbar:TBarButton Key="Cancel" Image="/hc_v4/img/ed_back.gif" Text="Cancel" ToolTip="Cancel" Visible="false"></igtbar:TBarButton>
					...
					<table cellspacing="0" cellpadding="0" width="100%" border="0">
						...
						<asp:Panel id="basicResourceInformation" runat="server">
							<tr valign="middle">
								...
								<td class="ugd"><A id="txtResourceURIValue" target="_blank" runat="server"></A></td>
							</tr>
						</asp:Panel>
						<asp:Panel id="linkAssociation" runat="server">
	                        <tr valign="middle">
							    <td class="editOptionalLabelCell" width="130">
								    <asp:label id="lbResSource" Text="Image Association" Runat="server"></asp:label></td>
							    <td class="ugd">
								    <asp:dropdownlist id="txtResSource" runat="server"></asp:dropdownlist>
							    </td>
						    </tr>
						</asp:Panel>
						</table>
				...
				<asp:Panel id="variantsPanel" runat="server">
					<FIELDSET>
						...
						<asp:DataGrid id="variantsGrid" runat="server" Width="100%" BorderColor="LightGray" DataKeyField="Id"
							AllowSorting="True" AllowPaging="True" EnableViewState="true" AutoGenerateColumns="False"
							...
							<Columns>
								...
								<asp:TemplateColumn SortExpression="Name">
									...
									<ItemTemplate>
										<a href="<%# ((HyperCatalog.Business.DAM.Variant)Container.DataItem).URI +"&j="+ rn.Next(500).ToString()%>" target="_blank"><img src="<%# ((HyperCatalog.Business.DAM.Variant)Container.DataItem).URI +"&j="+ rn.Next(500).ToString() %>&thumbnail=1&size=40" width="40" height="40" border="0"/></a>
									</ItemTemplate>
								</asp:TemplateColumn>
								<asp:TemplateColumn>
									<HeaderStyle HorizontalAlign="center"></HeaderStyle>
								...
								<asp:TemplateColumn>
									...
									<ItemTemplate>
										<asp:repeater id="attributesGrid" Runat="Server" DataSource='<%#((HyperCatalog.Business.DAM.Variant)Container.DataItem).Attributes%>'>
										  <HeaderTemplate><table cellpadding="0" cellspacing="0"><tr></HeaderTemplate>
											<ItemTemplate><td style="font-weight:bold;"><%# (string)Container.DataItem%>:</td><td>&nbsp;<%# ((HyperCatalog.Business.DAM.VariantAttributeSet)((System.Web.UI.WebControls.Repeater)Container.Parent).DataSource)[(string)Container.DataItem]%></td></ItemTemplate>
